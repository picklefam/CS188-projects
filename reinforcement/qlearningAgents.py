...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if legalMoves:
            return max([self.getQValue(state, move) for move in legalMoves])
        return 0.0
    ...
    def computeActionFromQValues(self, state):
        ...
        legalMoves = self.getLegalActions(state)
        if legalMoves:
            return max(legalMoves, key=lambda a: self.getQValue(state, a))
        return None
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.q_values[(state, action)] = oldValue * (1 - self.alpha) + newValue * self.alpha
